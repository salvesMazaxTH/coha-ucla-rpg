...
```js
context.registerResourceChange({ target, amount });
```
...
Hoje esse registro é feito por `context.registerResourceChange({ target, amount })`, que cobre ganho e gasto pelo sinal do valor.
