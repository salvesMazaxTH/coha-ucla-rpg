...
const TIMING = {
    ...
    DIALOG_DISPLAY: 750, // Reduced from 1200
    DIALOG_LEAVE: 80, // Reduced from 180

    // Sequencing gaps
    BETWEEN_EFFECTS: 20, // Reduced from 120
...
function getChampionElement(championId) {
    return document.querySelector(
        `.champion[data-champion-id="${championId}"]`
    );
}
...
export function createCombatAnimationManager(deps) {
    ...
    function shouldShowActionDialog(effect) {
        ...
        return ["damage", "evasion", "immune", "shieldBlock"].includes(
            effect.type
        );
    }
    ...
    function updateVisualResource(championId, delta, resourceType) {
        ...
        fill.style.background =
            resourceType === "energy" ? "#f4d03f" : "#4aa3ff";
        ...
        for (const champData of champions) {
            ...
            if (!champion) {
                ...
                try {
                    ...
                    console.error(
                        "[AnimManager] Failed to create champion:",
                        err
                    );
                    continue;
                }
